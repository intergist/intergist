...
### 1.1 Purpose
Board Reader Pro is a mobile training app based on the board-reading drill described in Angel Largay's *No-Limit Texas Hold'em: A Complete Course*[1]. The app displays five random community cards ("the board") and challenges the user to identify and rank the best possible two-card holdings (hole cards) from best to Nth best. It tracks performance over time so users can measure improvement in speed and accuracy.
...
**Layout:**
- Grid of 7 columns × 8 rows (52 cards minus 5 on board = 47, with 5 empty slots)
...
- Disabled (grayed) until the user has selected the required number of holdings.
- Label: "Submit [N] Holdings" with a checkmark icon.
- Tapping triggers a confirmation dialog only if fewer than the target number of holdings are selected: "You've ranked [X] of [Target]. Submit anyway?"
...
- Top-right icons:
  ...
  - **Hint** (💡): reveals the best possible hand category achievable on this board (e.g., "Best possible: Straight Flush"). Costs 1 hint credit; using a hint flags the drill as "assisted" in stats.
...
|---|---|---|
| Card Deck Theme | Classic, Modern, Four-Color | Four-Color |
...
- **2 points**: Correct holding at the exact correct rank.
- **1 point**: Correct holding present in the user's list but at the wrong rank, OR a holding that ties with the correct holding at that rank.
...
- The user receives full credit regardless of internal ordering among tied holdings.

### 5.4 Speed Score (Optional Gamification)
- A separate "speed rating" from 1–5 stars based on time:
...
| Card Green (♣ four-color) | #388E3C | #66BB6A |
...
4. Auto-select ♥ (only suit available for T after board cards removed) → T♥ selected
