...
2. The user selects and ranks two-card holdings from the remaining 47 cards, ordered from strongest (#1) to weakest (#N). Holdings that produce identical hand strength form an equivalence class and share the same rank — the user ranks classes, not individual holdings.
...
2. For each holding H, evaluate the best 5-card poker hand from the 7 cards (B ∪ H) and compute a numeric strength value.
...
Note: On this board, each top class has a single member because the heart suit is what creates the dominant hands. On a rainbow board, many of these classes would contain multiple members.
...
| 3.0 | March 23, 2026 | Consolidated CR-001 equivalence grouping throughout (see below) |

**Changes in v3.0 (CR-001 consolidation):**

1. **Section 1.1 — Purpose updated**: Added equivalence grouping to the purpose statement — users rank groups, not individual hands.
2. **Section 2.1 — Drill flow updated**: Steps 2 and 4 now describe class-based ranking and scoring.
3. **Section 2.3 — New section "Equivalence Grouping"**: Defines the core concept, key rules (strength-based grouping, sequential class ranks, duplicate-class blocking), and suit sensitivity.
4. **Section 3.1 — Screen map updated**: Drill Configuration references "classes to rank"; Results references class-based comparison and missed classes; Stats includes suit sensitivity.
5. **Section 4.2 — Drill Configuration updated**: "Holdings to Rank" renamed to "Classes to Rank"; Deal behavior includes pre-computing equivalence classes.
6. **Section 4.3.2 — Card picker updated**: Both modes now include duplicate-class blocking behavior and group-acceptance toasts.
7. **Section 4.3.3 — Holdings list updated**: Each entry displays equivalence class label instead of raw cards for multi-member classes. Full labeling hierarchy documented.
8. **Section 4.4 — Results screen rewritten**: Comparison table is now one row per equivalence class; scoring colors reference class matching; missed holdings section replaced with missed classes; suit-sensitivity note added to board texture section.
9. **Section 4.5 — Stats updated**: Added suit sensitivity metric card; history entries include suit-sensitive flag.
10. **Section 4.6 — Settings updated**: "Target Holdings" renamed to "Target Classes".
11. **Section 5 — Scoring algorithm rewritten**: Correct ranking generation now produces equivalence classes (§5.1); new section for class labeling (§5.2); scoring operates per-position against classes (§5.3); speed score references classes not holdings (§5.4).
12. **Section 6.5 — Animations updated**: Added duplicate-class blocked animation and class-based results reveal.
13. **Section 7.1 — Data model updated**: Added EquivalenceClass entity; Drill now stores correct_classes and suit_sensitive flag; RankedHolding references its equivalence_class.
14. **Section 8.2 — Performance updated**: Evaluation target now includes equivalence class computation; noted grouping is O(N).
15. **Section 8.4 — Error handling updated**: Duplicate card prevention replaced with duplicate-class blocking at the equivalence class level.
16. **Appendix A — Walkthrough updated**: Now shows equivalence class labels and includes a rainbow board example demonstrating multi-member classes.
17. **Appendix B — New**: Equivalence grouping test cases (4 scenarios covering rainbow boards, suit-sensitive boards, straight flush splits, and reduced combo counts).
18. **Appendix C**: Wireframes renumbered (previously Appendix B).
